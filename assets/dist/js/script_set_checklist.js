...
let url              = $('#te_url').val(); 
  ...
  function list_subroom(flg,veh_title_seq){
    $("#table-subroom > tbody").empty(); 
      if(veh_title_seq ){
                $.ajax({
                    url    : url+"Mainapp/ajax_list_data_checklist_items",   
                    data : { veh_title_seq }, 
                    type  : "POST",
                    dataType: "json",
...
//open modal add
  function modal_type(head,c,flg,veh_title_seq){
     ...
     //หัว modal
      ...
      //------------- ----------------------------------------
      $("#md_veh_title_seq").val(veh_title_seq).trigger('change'); 
  ...
  let flg= $("#hd_flg").val();
     ...
     if (md_veh_title_seq != ""  && flg_val == "Y"){

            $("#md_veh_title_seq").prop("disabled", false);

        $.ajax({
          url    : url+"Mainapp/ajax_save_checklist_items",
            type: 'POST',
            dataType: "json",
